...
def generate_caption(image_np):
    ...
    response = model.generate_content([
        "Give exactly one short Instagram-style caption (not a list) for this image:",
...
if uploaded_file:
    ...
    if st.button("Suggest Songs"):
        ...
        # --- Export Section ---
    st.markdown("## 🚀 Export to Social Media")

if st.button("Prepare for Sharing"):
        try:
            # Save edited image
            export_img = Image.fromarray(result)
            export_buffer = io.BytesIO()
            export_img.save(export_buffer, format="PNG")
            export_bytes = export_buffer.getvalue()

            # Generate caption and songs again if not already done
            caption = generate_caption(result)
            img_pil = Image.fromarray(result)
            buffer = io.BytesIO()
            img_pil.save(buffer, format="PNG")
            ...
            songs = song_response.text.strip()

            # Combine message
            full_message = f"{caption}\n\n🎶 Song Suggestions:\n{songs}"

            # Display and allow copy
            st.success("📤 Ready to share!")
            st.download_button("⬇️ Download Image", data=export_bytes, file_name="edited_image.png", mime="image/png")
            st.text_area("📋 Caption + Songs", value=full_message, height=150)

            # WhatsApp Share URL (opens WhatsApp Web)
            whatsapp_message = full_message.replace('\n', '%0A')
            ...
            # Instagram note (no direct sharing)
            st.info("📷 To post on Instagram, download the image and paste the caption manually on the Instagram app.")
